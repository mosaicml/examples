...
class ReplitModelHandler():
    ...
    def __init__(self, model_name: str = 'replit/replit-code-v1-3b'):
    ...
    def predict(self, model_requests: List[Dict]):
        ...
        input_tokens = self.tokenizer.encode(generate_inputs, return_tensors='pt', device=self.device)
        output = self.model.generate(input_tokens, **generate_kwargs)

        outputs = []
        for output in outputs:
            generated_code = self.tokenizer.decode(output, skip_special_tokens=True, clean_up_tokenization_spaces=False)
