...
def main(args):
    cfg_path = "yamls/main/rts/base.yaml"
    config = RunConfig.from_file(cfg_path)

    for seed in args.seeds:
        run = copy.deepcopy(config)

        run.name = f"rts-{args.scheduler}-{args.initial_mlm_rate}-to-{args.final_mlm_rate}-seed-{seed}"
        run.name = run.name.replace(".", "")
        rn = f"rts-schedule-{args.scheduler}-{args.initial_mlm_rate}-{args.final_mlm_rate}"
        if args.model == "large":
        ...
        run.parameters["loggers"]["wandb"]["tags"] = [
            "rts", "pretraining", args.scheduler,
            f"initial-{args.initial_mlm_rate}", f"final-{args.final_mlm_rate}",
            f"bert-{args.model}"
        ]
        ...
        if args.autoresume:
            ...
            if args.load_ckpt is not None:
                run.parameters["load_path"] = f"s3://mosaicml-internal-checkpoints-shared/zack/rebuttal/{rn}-seed-{seed}/ep0-ba{args.load_ckpt}-rank0.pt"
            else:
                run.parameters["load_path"] = f"s3://mosaicml-internal-checkpoints-shared/zack/rebuttal/{rn}-seed-{seed}/latest-rank0.pt.symlink" 
                # print(run.parameters["load_path"])
        ...
        print("Launched run: ", launched_run.name)
