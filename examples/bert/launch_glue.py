...
def build_checkpoint_path(scheduler, init_rate, final_rate, seed, ckpt, model):
    return f"s3://mosaicml-internal-checkpoints-shared/zack/paper-mlm-schedule/{'large-' if model == 'large' else ''}schedule-{scheduler}-{init_rate}-{final_rate}-seed-{seed}/ep0-ba{ckpt}-rank0.pt"
...
def main(args):
    ...
    for ckpt in ckpts:
        for seed in [17, 2048, 3047]:
            ...
            rn = f"schedule-{args.scheduler}-{args.initial_mlm_rate}-{args.final_mlm_rate}-seed-{seed}-ckpt-{ckpt}"
            if args.model == "large":
                rn = "large-" + rn
            run.parameters["base_run_name"] = rn

            # if args.mode == "speed-up" or args.priority == "low":
            #     run.scheduling = {"priority": "low"}
