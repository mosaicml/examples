...
def test_trainer(use_recipe):
    ...
    with SynthClassificationDirectory(
    ) as tmp_datadir, SynthClassificationDirectory() as tmp_datadir2:
        config.train_dataset.path = tmp_datadir
        config.train_dataset.local = tmp_datadir
        config.eval_dataset.path = tmp_datadir2
        config.eval_dataset.local = tmp_datadir2
