...
class GPTFakeTPMLP(nn.Module):
    ...
    def forward(self, x):
        bs, ndim = x.shape[0], x.ndim
        repeats = [self.tp_world_size] + [1] * (ndim - 1)
        fake_all_gather_x = x.repeat(repeats)
        out = self.mlp_down(self.mlp_act(self.mlp_up(fake_all_gather_x)))
        fake_reduce_scatter_out = out[:bs]
...
class MosaicGPT(nn.Module):
    ...
    def fsdp_wrap_fn(self, module):
        if isinstance(module, GPTBlock):
            return True
        if isinstance(module, GPTFakeTPMLP):
            local_world_size = get_local_world_size()
            local_rank = get_local_rank()
            world_size = get_world_size()
            ranks = [
                local_rank + local_world_size * n
                for n in range(world_size // local_world_size)
            ]
            return {'process_group': torch.distributed.new_group(ranks=ranks)}
