...
class MixtureOfDenoisersPrinterCallback(Callback):
    ...
    def init(self, state: State, logger: Logger) -> None:
        ...
        if not hasattr(state.model, 'model'):
            raise AttributeError(
                'MixtureOfDenoisersPrinterCallback callback expects state.model to have attribute `model`.'
            )
        hf_model = getattr(state.model, 'model')
        if isinstance(hf_model, FullyShardedDataParallel):
            hf_model = getattr(hf_model, 'module')
            self._using_fsdp = True
        if not isinstance(hf_model, PreTrainedModel):
            raise TypeError(
                'MixtureOfDenoisersPrinterCallback only supports models that subclass `transformers.PreTrainedModel`'
            )
    ...
    def after_dataloader(self, state: State, logger: Logger) -> None:
        ...
        if (dist.get_global_rank() != 0) and (not self._using_fsdp):
            return

        if not self._verified_collator:
            collate_fn = state.train_dataloader.collate_fn
            if not isinstance(collate_fn, MixtureOfDenoisersCollator):
                raise TypeError(
                    'Use of `MixtureOfDenoisersPrinterCallback` is only supported when the dataloader ' +\
                    'uses the `MixtureOfDenoisersCollator` collate function.'
                )
            self._verified_collator = True
            self._decoder_only_format = bool(collate_fn.decoder_only_format)
        ...
        input_str, target_str, output_str = self._generate_input_target_output_strings(
            ...
            decoder_only_format=self._decoder_only_format,
            generate_kwargs=self._generate_kwargs,
            sync_gpus=self._using_fsdp)
        ...
        if dist.get_global_rank() == 0:
            try:
                ...
                if self._raise_on_error:
                    raise
                print(
                    f'\n\n{self.__class__.__name__}: Failed to parse input/target/output strings. ' +\
                    '\n\tThis is expected if the model is early in training. ' +\
                    '\n\tSet `raise_on_failure=True` if you want this to failure generate an error.\n',
                    file=sys.stdout,
                    flush=True)
    ...
    def _generate_input_target_output_strings(
        batch: Dict[str, torch.TensorType],
        ...
        if decoder_only_format:
            ...
            if sync_gpus:
                # Hack for FSDP (use sync_gpus=True with FSDP wrapping)
                # This call avoids friction beween model.generate and FSDP.
                _ = model(input_ids[:, :1])
            outputs = model.generate(input_ids,
                                     max_length=max_new_tokens,
                                     pad_token_id=tokenizer.pad_token_id,
                                     synced_gpus=sync_gpus,
                                     **generate_kwargs)
            ...
            if sync_gpus:
                # Hack for FSDP (use sync_gpus=True with FSDP wrapping)
                # This call avoids friction beween model.generate and FSDP.
                _ = model(input_ids[:, :1], decoder_input_ids=labels[:, :1])
            outputs = model.generate(input_ids,
        ...
        target_str = tokenizer.decode(labels[0])
