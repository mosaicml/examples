...
try:
    import fused_dgd_lib as fused_dense_cuda  # type: ignore
...
class FusedDenseResGeluDenseFunc(torch.autograd.Function):
    ...
    def backward(ctx, grad_output, grad_input):
        ...
        return grad_input.reshape_as(
            x), grad_weight1, grad_bias1, grad_weight2, grad_bias2, None
...
class DenseResGeluDense(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc1 = nn.Linear(in_features,
                             ...
                             bias=bias,
                             **factory_kwargs)
        self.fc2 = nn.Linear(intermediate_features,
                             out_features,
                             bias=bias,
                             **factory_kwargs)
    ...
    def forward(self, x):
        return fused_dense_res_gelu_dense_function(x, self.fc1.weight,
                                                   self.fc1.bias,
                                                   self.fc2.weight,
                                                   self.fc2.bias)
