...
class BranchingGPT(nn.Module):

    def __init__(self, cfg: DictConfig):
        super().__init__()
        assert cfg.name == 'branching_gpt', f'Tried to build BranchingGPT model with cfg.name={cfg.name}'
        self.cfg = cfg
        ...
        self.num_branches = cfg.get('num_branches', 0)
        self.final_loss_weight = cfg.get('final_loss_weight', 0.9)
    ...
    def forward(self,
        ...
        early_branches = []
        branch_every_n_idxs = len(self.transformer.blocks) //  (self.num_branches + 1) 
        for idx, block in enumerate(self.transformer.blocks):  # type: ignore
            x = block(
                x, None if self.cfg.attn_impl == 'triton' else key_padding_mask,
                attn_mask)

            if idx % branch_every_n_idxs == 0 and idx > 0 and idx < len(self.transformer.blocks) - 1:
                early_branches.append((idx, x))


        x = self.transformer.ln_f(x)  # type: ignore
        early_branches = [(idx, self.transformer.ln_f(y)) for idx, y in early_branches]
        # output embedding weight tied to input embedding
        assert isinstance(self.transformer.wte, nn.Module)  # pyright
        assert isinstance(self.transformer.wte.weight, torch.Tensor)  # pyright
        logits = F.linear(x, self.transformer.wte.weight, None)
        early_branches = [(idx, F.linear(y, self.transformer.wte.weight, None)) for idx,y in early_branches]

        return logits, early_branches
...
class ComposerBranchingGPT(ComposerModel):

    def __init__(self, cfg):
        super().__init__()
        self.model = BranchingGPT(cfg)
    ...
    def weighted_cross_ent(self, final_cross_ent, early_cross_ents):
        early_loss = sum([cross_ent for _, cross_ent in early_cross_ents]) / len(early_cross_ents)
        return self.model.final_loss_weight * final_cross_ent + (1-self.model.final_loss_weight) * early_loss

    def loss(self, outputs, batch):

        targets = self.get_targets(batch)
        final_output, early_outputs = outputs
    ...
    def update_metric(self, batch, outputs, metric):
        ...
        final_output = final_output.view(-1, final_output.size(-1))
        targets = self.get_targets(batch).view(-1)
        metric.update(final_output, targets)
