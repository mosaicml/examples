...
class TwoBitLionW(Optimizer):
    ...
    @staticmethod
    def lionw(p, grad, prev_grad, maj_vote, step, lr, initial_lr, wd) -> None:
        # stepweight decay
        if wd != 0:
            decay_factor = (lr / initial_lr) if initial_lr else 1.0
            p.data.mul_(1 - decay_factor * wd)
        ...
        # update is interpolation between gradient and momentum
        update = (update_bool.int() - 0.5) * 2.0
        p.add_(update, alpha=-lr)

        # momentum is interp b/w gradient and itself
        maj_vote.data.copy_(update_bool)
        prev_grad.data.copy_(grad_bool)
    ...
    def step(self, closure: Optional[Callable] = None):
        ...
        for group in self.param_groups:
            for p in filter(lambda p: p.grad is not None and p.requires_grad,
                ...
                grad, lr, initial_lr, wd, state = p.grad, group[
                    'lr'], group['initial_lr'], group[
                        'weight_decay'], self.state[p]

                # init state - exponential moving average of gradient values

                if len(state) == 0:
                    state['majority_vote'] = torch.zeros_like(p, dtype=torch.bool)
                    state['prev_grad'] = torch.zeros_like(p, dtype=torch.bool)
                    state['step'] = 0


                prev_grad = state['prev_grad']
                maj_vote = state['majority_vote']
                step = state['step']
                self.lionw(p, grad, prev_grad, maj_vote, step, lr, initial_lr, wd)
                state['step'] += 1
