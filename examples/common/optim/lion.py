...
class AdaptiveWDLionW(Optimizer):
    metric_functions = {
        'l2_norm/moment':
            lambda param, optim_state, step_tensor: torch.linalg.vector_norm(optim_state['exp_avg']),
        'l2_norm/param':
    ...
    def __init__(
        ...
        self.wd_min_fraction = wd_min_fraction
        self.wd_max_fraction = wd_max_fraction
        self.wd_scaling_slope = wd_scaling_slope
    ...
    def step(
        ...
        for p in tmp_param_current_norm:
            ...
            wd_scale = torch.tensor((p_t-p_0)/(self.wd_scaling_slope*p_0)).clamp(self.wd_min_fraction, self.wd_max_fraction)
    ...
    def dist_reduce_metrics(self, optimizer_metrics):
        for metric in optimizer_metrics:
            if metric.startswith('l2_norm'):
                ...
                B_reduced_norm = optimizer_metrics[f'l2_norm/{B}/{layer}']
                optimizer_metrics[metric] = reduced / (A_reduced_norm * B_reduced_norm)
            elif metric.startswith('wd_scaling'):
                continue
            else:
                reduced = optimizer_metrics[metric]
                if dist.get_world_size() > 1:
    ...
    def report_per_parameter_metrics(self, param: torch.Tensor, name: str, optimizer_metrics: dict):
        lr = self.param_groups[0]['lr']
        weight_decay = lr * self.param_wd_scaling.get(param, 0.0)
        initial_lr = self.param_groups[0]['initial_lr']
...
class WeightNormLion(Optimizer):
    metric_functions = {
        'l2_norm/moment':
            lambda param, optim_state, step_tensor: torch.linalg.vector_norm(optim_state['exp_avg']),
        'l2_norm/param':
