...
class MaskAdam(AdamW):
    ...
    def report_per_parameter_metrics(self, param: torch.Tensor, name: str, optimizer_metrics: dict):
        ...
        if param in self.state:
            ...
            if step >= self.warmup:
                update = param_optim_state['exp_avg'].sign().mul_(param.grad.sign()).sign_().clamp_(0)
                update.mul_(param_optim_state['exp_avg']).mul_(beta1).add_(param.grad, alpha=1 - beta1)
            else:
                update = param_optim_state['exp_avg'].mul(beta1).add_(param.grad, alpha=1 - beta1)

            step_tensor = step_size * update.div(denom)
            decay_factor = (lr / initial_lr) if initial_lr else 1.0
