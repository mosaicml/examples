...
with open(os.path.join(os.path.dirname(path_to_current_file), os.pardir, 'test_ticker_to_years.json'), 'r') as _json_file:
    ticker_to_years = {k: sorted(v, key=lambda x: int(x)) for k, v in json.load(_json_file).items()}
...
def greet(
    ...
    ticker = ticker.upper()
    remote_file_path = os.path.join(args.remote_folder_path, ticker, f'sec_{year}_txt.txt')
    local_file_path = os.path.join(os.getcwd(), 'local-data', ticker, f'sec_{year}_txt.txt')
    ...
    for batch in tqdm(batches, desc='Embedding documents', total=len(batches)):
        batch_embeddings = embeddings.embed_documents([d.page_content for d in batch])
        text_embeddings.extend(list(zip([d.page_content for d in batch], batch_embeddings)))
    ...
    return answer, cite, '\n\n'.join([d.page_content for d in answer_response['source_documents']])
...
with gr.Blocks() as demo:
    ...
    query_btn.click(fn=greet, inputs=[ticker, year, query], outputs=[answer, quote, sources])
