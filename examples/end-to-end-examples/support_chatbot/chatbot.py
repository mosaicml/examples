...
class RetrieverWithScore(BaseRetriever):
    """Just a custom retriever to track distance between query and retrieval
    docs.
    ...
    def __init__(self, search_type: str, vector_store: FAISS, k: int,
                 score_threshold: int):
        self.k = k
    ...
    def _get_relevant_documents(self, query: str) -> list[Document]:
        ...
        if self.search_type != 'similarity':
            raise ValueError(
                f"Only search_type='similarity' is supported with scores")
        docs_and_scores = self.vector_store.similarity_search_with_score(
            query=query, k=self.k, score_threshold=self.score_threshold)
...
PARTIAL_SUBQA_TEMPLATE = ("""<|im_start|>system
                             ...
                             <|im_start|>assistant""")
PARTIAL_COMBINE_TEMPLATE = (
    """<|im_start|>system A conversation between a user and an LLM-based AI assistant.
...
class ChatBot:
    """Given a folder of .txt files from data_path, create a Chatbot object that
    can process the files into documents, split them into managable sizes, and
    store them in a vector store. The Chatbot can then be used to answer
    ...
    def __init__(
        ...
        self.saved_state = {
            ...
            'model_k': model.model_kwargs['top_k']
        }
        ...
        if os.path.isfile(os.path.join(data_path, 'vectors.pickle')):
            with open(os.path.join(self.data_path, 'vectors.pickle'),
                      'rb') as f:
                self.vector_store = pickle.load(f)

    def load_data(self) -> list[Document]:
        """Given a directory find all .txt files and load them as documents into
        a list.
            ...
            for filename in filenames:
                if filename.endswith('.txt'):
                    ...
                    document.metadata = {
                        ...
                        **{
                            'file_name':
                                filename.replace('{slash}', '/').replace(
                                    '{dot}', '.').replace('{colon}', ':')[:-4]
                        }
                    }
    ...
    def split_pages(self, pages: list[Document]) -> list[Document]:
        """Given a list of documents split them into smaller documents of size
        `self.chunk_size`
    ...
    def documents_to_str(self, documents: list[Document]) -> list[str]:
        return map(lambda doc: doc.page_content, documents)

    def clean_response(self, input_text: str) -> str:
        """Clean the response from the model by stripping some bad answer
        prefixes, new lines, etc.
    ...
    def store_vectors(self, pages: list[Document]) -> None:
        """Given a list of documents, split them into chunks, and store them in
        a vector store.
        ...
        for batch in tqdm(content_batches,
                          desc='Embedding documents',
                          total=len(content_batches)):
            batch_embeddings = self.embedding.embed_documents(
                [p.page_content for p in batch])
            txt_embeddings.extend(
                list(zip([p.page_content for p in batch], batch_embeddings)))
        ...
        with open(os.path.join(ROOT_DIR, 'retrieval_data/vectors.pickle'),
                  'wb') as f:
            pickle.dump(vector_store, f)
            self.vector_store = vector_store

    def create_vector_store(self, repository_urls) -> None:
        """Download the repositories, load the data, split the data into chunks,
        and store the chunks in a vector store.
        ...
        for repo_url in repository_urls:
            downloader = RepoDownloader(output_dir=self.data_path,
                                        current_dir='',
                                        repo_url=repo_url)
    ...
    def normalize_str(self, answer: str):
    ...
    def set_eval_state(self) -> None:
        """Set the state of the chatbot to the evaluation state.

        This is used to change the chunk size, chunk overlap, and k
        """
    ...
    def reload_chat_state(self) -> None:
        """Reload the chatbot state to the saved state the user set when
    ...
    def evaluate_simple(self, data_path: str,
                        answer_question_string_template: str) -> str:
        """Evaluate the chatbot on simple retrieval dataset given a data_path
        and a chain.
        ...
        chain = self.create_chain(
            prompt_template=answer_question_string_template)
        exact_match = 0
        close_match = 0
        total = 1
        ...
        with open(data_path, 'r') as file:
            for line in tqdm(file, total=total_lines, desc='Processing lines'):
                ...
                if self.normalize_str(answer) == self.normalize_str(
                        continuation):
                    exact_match += 1
                elif self.normalize_str(continuation).replace(
                        ' ', '') in self.normalize_str(answer).replace(' ', ''):
                    close_match += 1
                else:
                    print('\n', self.normalize_str(answer), '||',
                          self.normalize_str(continuation), '\n')
                    print(
                        f'{exact_match} exact matches and {close_match} close matches out of {total} questions.'
                    )
                total += 1
                time.sleep(0.5)
    ...
    def evaluate_complex(self, data_path: str,
                         answer_question_string_template: str) -> str:
        """Evaluate the chatbot on complex eval dataset given a data_path and a
        chain.
    ...
    def sub_query_chat(self, query: str, threshold=0.4) -> str:
        if not self.intent_chain:
            save_k = self.k
            self.k = 5
            self.intent_chain = self.create_chain(
                prompt_template=SUBQUERY_INTENT_TEMPLATE)
            self.k = save_k
        intent_response = self.intent_chain(query)
        intent_answer = self.clean_response(
            intent_response['result'].lstrip('\n'))
        ...
        subQA_answer = self.clean_response(
            subQA_response['result'].lstrip('\n'))
        ...
        for sub_QA in all_sub_QA:
            if sub_QA:
                ...
                answer = self.clean_response(response['result'].lstrip('\n'))
                if response['source_documents'] and response[
                        'source_documents'][0].metadata['score'] > threshold:
                    answer = self.clean_response(
                        response['result'].lstrip('\n'))
                    sub_QA_injection += f'Question: {sub_QA} \nAnswer: {answer}\n'

        if sub_QA_injection:
            SUBQUERY_COMBINE_TEMPLATE = PARTIAL_COMBINE_TEMPLATE.format(
                str(sub_QA_injection).replace('{', '{{').replace('}', '}}'))
            combine_chain = self.create_chain(
                prompt_template=SUBQUERY_COMBINE_TEMPLATE)
            combine_response = combine_chain(query)
            combine_answer = self.clean_response(
                combine_response['result'].lstrip('\n'))
    ...
    def relation_sub_query_chat(self, query: str, threshold: int = 0.4) -> str:
        if not self.intent_chain:
            save_k = self.k
            self.k = 3
            self.intent_chain = self.create_chain(
        ...
        if not self.subsubchain:
            save_k = self.k
            self.k = 2
            self.subsubchain = self.create_chain(
                prompt_template=SUBQUERY_RELATED_TEMPLATE, score_threshold=0)
            self.k = save_k
        for sub_QA in all_sub_QA:
            if sub_QA:
                answerable = self.clean_response(
                    self.subsubchain(sub_QA)['result'].lstrip('\n'))
                if 'Yes' in answerable:
                    if not self.subchain:
                        self.subchain = self.create_chain(
                            prompt_template=CHAT_30B_TEMPLATE)
                    response = self.subchain(sub_QA)
                    answer = self.clean_response(
                        response['result'].lstrip('\n'))
                    sub_QA_injection += f'Question: {sub_QA} \nAnswer: {answer}\n'
    ...
    def chat(self, query: str) -> str:
        ...
        if not self.chat_chain:
            self.chat_chain = self.create_chain(
                prompt_template=CHAT_30B_TEMPLATE, score_threshold=0)
