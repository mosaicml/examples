...
def build_dummy_stable_diffusion_model(model_name_or_path: str,
    ...
    unet = UNet2DConditionModel(
        **PretrainedConfig.get_config_dict(model_name_or_path, subfolder='unet')[0])
    if is_xformers_available():
        unet.enable_xformers_memory_efficient_attention()
    vae = AutoencoderKL(
        **PretrainedConfig.get_config_dict(model_name_or_path, subfolder='vae')[0])

    text_encoder = CLIPTextModel(config=PretrainedConfig.from_pretrained(model_name_or_path, subfolder='text_encoder'))
...
def test_model_builder(model_name):
    """
    test that the StableDiffusion base class outputs the correct size outputs for both model versions
    """
