...
def build_cifar10_dataspec(
    ...
    **dataloader_kwargs: Any,
    ...
    Args:
        data_path (str): Path to the data either stored locally or remotely (e.g. in a S3 bucket)
        is_streaming (bool): Whether the data is stored locally or remotely (e.g. in a S3 bucket)
        batch_size (int): Batch size per device.
        local (str, optional): If using streaming, local filesystem directory where data is cached during operation.
            Default: ``None``.
        is_train (bool): Whether to load the training data or validation data. Default:
            ``True``.
        download (bool, optional): Whether to download the data locally, if needed. Default:
            ``True``.
