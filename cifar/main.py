...
def main(config):
    ...
    if dist.get_world_size() > 1:
        dist.initialize_dist(device, 120)
    ...
    train_dataspec = build_cifar10_dataspec(data_path=config.train_dataset.path,
    ...
    test_dataspec = build_cifar10_dataspec(data_path=config.eval_dataset.path,
    ...
    model = build_composer_resnet_cifar(model_name=config.model.name, num_classes=config.model.num_classes)
    ...
    lr_scheduler = MultiStepWithWarmupScheduler(t_warmup=config.lr_scheduler.t_warmup,
